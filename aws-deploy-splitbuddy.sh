...
# SplitBuddy Backend AWS Deployment Script
# Configure your AWS credentials before running this script
...
echo -e "${GREEN}🚀 Starting SplitBuddy Backend AWS Deployment${NC}"
...
# Step 6: Create or update ECS cluster
...
cat > task-definition-splitbuddy.json << EOF
{
...
aws ecs register-task-definition --cli-input-json file://task-definition-splitbuddy.json --region $AWS_REGION
...
if ! aws ecs describe-services --cluster $CLUSTER_NAME --services $SERVICE_NAME --region $AWS_REGION --query 'services[0].status' --output text &> /dev/null || [ "$(aws ecs describe-services --cluster $CLUSTER_NAME --services $SERVICE_NAME --region $AWS_REGION --query 'services[0].status' --output text)" = "None" ]; then
    ...
    echo ""
    warn "Or use Terraform to create the complete infrastructure:"
    echo "terraform init"
    echo "terraform plan"
...
# Cleanup
rm -f task-definition-splitbuddy.json
...
echo -e "${GREEN}🎉 SplitBuddy Backend deployment to AWS completed!${NC}"
...
echo -e "${YELLOW}   1. Set up RDS PostgreSQL database${NC}"
echo -e "${YELLOW}   2. Set up ElastiCache Redis cluster${NC}"
